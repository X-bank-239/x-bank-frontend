...
function ChartsContent({
  ...
  if (mode === "summary") {
    const hasAny = summaryByCurrency.some((b) => chartHasData(b.dayBars, b.categorySlices));
    if (!hasAny) {
  ...
  const hasAny = chartsByAccount.some((c) => chartHasData(c.dayBars, c.categorySlices));
  if (!hasAny) {
    return <p className="text-sm text-slate-500 dark:text-slate-400">{emptyMessage}</p>;
  }
...
/** @deprecated используйте AccountsSpendingCharts с variant="dashboard" */
